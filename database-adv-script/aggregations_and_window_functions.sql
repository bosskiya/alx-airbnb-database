...
SELECT
    ...
    ROW_NUMBER() OVER (ORDER BY total_bookings DESC) AS row_number_by_bookings
FROM (SELECT p.property_id, p.name AS property_name, COUNT(b.booking_id) AS total_bookings
